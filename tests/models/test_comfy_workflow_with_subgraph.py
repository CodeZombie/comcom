import json
import pytest

from comcom.comfy_ui.definition.node_definitions import NodeDefinitions
from comcom.comfy_ui.workflow_graph.workflow import ComfyWorkflow


COMFY_WORKFLOW_WITH_SUBGRAPH_JSON_STRING = '''
{
  "id": "941084b1-a392-44f4-9064-688e43b2ea48",
  "revision": 0,
  "last_node_id": 7,
  "last_link_id": 11,
  "nodes": [
    {
      "id": 1,
      "type": "LoadImage",
      "pos": [
        70,
        -140
      ],
      "size": [
        274.080078125,
        102
      ],
      "flags": {},
      "order": 0,
      "mode": 0,
      "inputs": [],
      "outputs": [
        {
          "name": "IMAGE",
          "type": "IMAGE",
          "links": [
            8
          ]
        },
        {
          "name": "MASK",
          "type": "MASK",
          "links": null
        }
      ],
      "properties": {
        "Node name for S&R": "LoadImage"
      },
      "widgets_values": [
        "my_cool_image.png",
        "image"
      ]
    },
    {
      "id": 2,
      "type": "CheckpointLoaderSimple",
      "pos": [
        70,
        -300
      ],
      "size": [
        270,
        98
      ],
      "flags": {},
      "order": 1,
      "mode": 0,
      "inputs": [],
      "outputs": [
        {
          "name": "MODEL",
          "type": "MODEL",
          "links": [
            10
          ]
        },
        {
          "name": "CLIP",
          "type": "CLIP",
          "links": null
        },
        {
          "name": "VAE",
          "type": "VAE",
          "links": [
            9
          ]
        }
      ],
      "properties": {
        "Node name for S&R": "CheckpointLoaderSimple"
      },
      "widgets_values": [
        "diffusion_model.safetensors"
      ]
    },
    {
      "id": 7,
      "type": "b48931b4-1499-46f5-85c3-1a5ef033a29b",
      "pos": [
        590,
        -280
      ],
      "size": [
        140,
        66
      ],
      "flags": {},
      "order": 2,
      "mode": 0,
      "inputs": [
        {
          "name": "pixels",
          "type": "IMAGE",
          "link": 8
        },
        {
          "name": "vae",
          "type": "VAE",
          "link": 9
        },
        {
          "name": "model",
          "type": "MODEL",
          "link": 10
        }
      ],
      "outputs": [
        {
          "name": "IMAGE",
          "type": "IMAGE",
          "links": [
            11
          ]
        }
      ],
      "properties": {},
      "widgets_values": []
    },
    {
      "id": 6,
      "type": "PreviewImage",
      "pos": [
        860,
        -280
      ],
      "size": [
        140,
        26
      ],
      "flags": {},
      "order": 3,
      "mode": 0,
      "inputs": [
        {
          "name": "images",
          "type": "IMAGE",
          "link": 11
        }
      ],
      "outputs": [],
      "properties": {
        "Node name for S&R": "PreviewImage"
      }
    }
  ],
  "links": [
    [
      8,
      1,
      0,
      7,
      0,
      "IMAGE"
    ],
    [
      9,
      2,
      2,
      7,
      1,
      "VAE"
    ],
    [
      10,
      2,
      0,
      7,
      2,
      "MODEL"
    ],
    [
      11,
      7,
      0,
      6,
      0,
      "IMAGE"
    ]
  ],
  "groups": [],
  "definitions": {
    "subgraphs": [
      {
        "id": "b48931b4-1499-46f5-85c3-1a5ef033a29b",
        "version": 1,
        "state": {
          "lastGroupId": 0,
          "lastNodeId": 6,
          "lastLinkId": 7,
          "lastRerouteId": 0
        },
        "revision": 0,
        "config": {},
        "name": "New Subgraph",
        "inputNode": {
          "id": -10,
          "bounding": [
            440,
            -204,
            120,
            100
          ]
        },
        "outputNode": {
          "id": -20,
          "bounding": [
            1270,
            -184,
            120,
            60
          ]
        },
        "inputs": [
          {
            "id": "6d48fa68-1a72-42fb-9e5b-26ed31682bbd",
            "name": "pixels",
            "type": "IMAGE",
            "linkIds": [
              4
            ],
            "localized_name": "pixels",
            "pos": {
              "0": 540,
              "1": -184
            }
          },
          {
            "id": "10c6b481-e7be-499c-b4be-7bc274fdde3f",
            "name": "vae",
            "type": "VAE",
            "linkIds": [
              3,
              6
            ],
            "localized_name": "vae",
            "pos": {
              "0": 540,
              "1": -164
            }
          },
          {
            "id": "74435c16-2d81-46f0-bbe6-90eefa31e687",
            "name": "model",
            "type": "MODEL",
            "linkIds": [
              1
            ],
            "localized_name": "model",
            "pos": {
              "0": 540,
              "1": -144
            }
          }
        ],
        "outputs": [
          {
            "id": "640f3519-fa22-41a1-8cbf-2dfa3d6371df",
            "name": "IMAGE",
            "type": "IMAGE",
            "linkIds": [
              7
            ],
            "localized_name": "IMAGE",
            "pos": {
              "0": 1290,
              "1": -164
            }
          }
        ],
        "widgets": [],
        "nodes": [
          {
            "id": 4,
            "type": "VAEEncode",
            "pos": [
              620,
              -60
            ],
            "size": [
              140,
              46
            ],
            "flags": {},
            "order": 1,
            "mode": 0,
            "inputs": [
              {
                "localized_name": "pixels",
                "name": "pixels",
                "type": "IMAGE",
                "link": 4
              },
              {
                "localized_name": "vae",
                "name": "vae",
                "type": "VAE",
                "link": 3
              }
            ],
            "outputs": [
              {
                "localized_name": "LATENT",
                "name": "LATENT",
                "type": "LATENT",
                "links": [
                  2
                ]
              }
            ],
            "properties": {
              "Node name for S&R": "VAEEncode"
            }
          },
          {
            "id": 3,
            "type": "KSampler",
            "pos": [
              780,
              -270
            ],
            "size": [
              270,
              262
            ],
            "flags": {},
            "order": 0,
            "mode": 0,
            "inputs": [
              {
                "localized_name": "model",
                "name": "model",
                "type": "MODEL",
                "link": 1
              },
              {
                "localized_name": "positive",
                "name": "positive",
                "type": "CONDITIONING",
                "link": null
              },
              {
                "localized_name": "negative",
                "name": "negative",
                "type": "CONDITIONING",
                "link": null
              },
              {
                "localized_name": "latent_image",
                "name": "latent_image",
                "type": "LATENT",
                "link": 2
              }
            ],
            "outputs": [
              {
                "localized_name": "LATENT",
                "name": "LATENT",
                "type": "LATENT",
                "links": [
                  5
                ]
              }
            ],
            "properties": {
              "Node name for S&R": "KSampler"
            },
            "widgets_values": [
              0,
              "randomize",
              20,
              8,
              "euler",
              "normal",
              1
            ]
          },
          {
            "id": 5,
            "type": "VAEDecode",
            "pos": [
              1070,
              -270
            ],
            "size": [
              140,
              46
            ],
            "flags": {},
            "order": 2,
            "mode": 0,
            "inputs": [
              {
                "localized_name": "samples",
                "name": "samples",
                "type": "LATENT",
                "link": 5
              },
              {
                "localized_name": "vae",
                "name": "vae",
                "type": "VAE",
                "link": 6
              }
            ],
            "outputs": [
              {
                "localized_name": "IMAGE",
                "name": "IMAGE",
                "type": "IMAGE",
                "links": [
                  7
                ]
              }
            ],
            "properties": {
              "Node name for S&R": "VAEDecode"
            }
          }
        ],
        "groups": [],
        "links": [
          {
            "id": 2,
            "origin_id": 4,
            "origin_slot": 0,
            "target_id": 3,
            "target_slot": 3,
            "type": "LATENT"
          },
          {
            "id": 5,
            "origin_id": 3,
            "origin_slot": 0,
            "target_id": 5,
            "target_slot": 0,
            "type": "LATENT"
          },
          {
            "id": 4,
            "origin_id": -10,
            "origin_slot": 0,
            "target_id": 4,
            "target_slot": 0,
            "type": "IMAGE"
          },
          {
            "id": 3,
            "origin_id": -10,
            "origin_slot": 1,
            "target_id": 4,
            "target_slot": 1,
            "type": "VAE"
          },
          {
            "id": 6,
            "origin_id": -10,
            "origin_slot": 1,
            "target_id": 5,
            "target_slot": 1,
            "type": "VAE"
          },
          {
            "id": 1,
            "origin_id": -10,
            "origin_slot": 2,
            "target_id": 3,
            "target_slot": 0,
            "type": "MODEL"
          },
          {
            "id": 7,
            "origin_id": 5,
            "origin_slot": 0,
            "target_id": -20,
            "target_slot": 0,
            "type": "IMAGE"
          }
        ],
        "extra": {}
      }
    ]
  },
  "config": {},
  "extra": {
    "ds": {
      "scale": 1.377743110045632,
      "offset": [
        359.751390578203,
        579.7503273547081
      ]
    },
    "frontendVersion": "1.24.0-1"
  },
  "version": 0.4
}
'''

@pytest.fixture
def node_definitions():
    return NodeDefinitions.model_validate_json(open('tests/data/object_info.json').read())

@pytest.fixture
def workflow():
    return ComfyWorkflow.model_validate_json(COMFY_WORKFLOW_WITH_SUBGRAPH_JSON_STRING)

def test_subgraph_data(workflow, node_definitions):
    assert workflow.id == "941084b1-a392-44f4-9064-688e43b2ea48"
    assert len(workflow.subgraphs) == 1

    # Check if the subgraph's Ksampler's "model" input is hooked up to the subgraph's 3rd input slot.
    subgraph = workflow.get_subgraph('b48931b4-1499-46f5-85c3-1a5ef033a29b')
    assert subgraph.get_node('3').get_link_input('model').link == '1'